        ...
        <div class="py-3 sm:max-w-4xl sm:mx-auto">
            ...
            <div class="bg-white shadow-lg border-gray-100 border sm:rounded-3xl p-8 flex space-x-8">
                ...
                <div class="flex flex-col w-1/2 space-y-4">
                    ...
                    <div class="relative flex">
                            <div class="mt-2 bg-green-400 font-bold rounded-xl p-2 text-white ">{{movie.userRating}}</div>
                            <i class="absolute left-14 fas fa-star fa-3x text-yellow-500"></i>
                            <div class="absolute left-14 pl-4 pt-3 font-bold "> {{ calRating }} </div>
                    </div>
                </div>
            </div>
            <!-- 댓글 구간 -->
            <div class="bg-white border-gray-100 sm:rounded-3xl p-8 mt-2">
                ...
                <div class="text-center">
                    <button v-if="current_page!=1" @click="current_page+=-1,CommentInfo()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded center"> &lt; </button>
                    <button v-for="i in last_page" :key="i" @click="[current_page=i,CommentInfo()]" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded center"> <div v-if="current_page != i">{{ i }} </div></button>
                    <button v-if="current_page!=last_page" @click="current_page+=1,CommentInfo()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded center"> &gt; </button>
                </div>
               <!-- 댓글 -->
               <div v-for="comment in comments.data" :key="comment" class="bg-gray-200 rounded-lg p-3 flex flex-col justify-center items-center md:items-start shadow-lg mb-4 ">
                    <div class="flex flex-row justify-center mr-2 ">
                        <img  width="48" height="48" class="rounded-full w-10 h-10 mr-4 shadow-lg mb-2" :src="`${comment.user_imagePath}`" :alt="`${comment.user_name}`">
    ...
    <dialog-modal :show="showmodal">
        <template #content>
            <div v-if="check==0">
                ...
                <button @click="movieCheck()">인증하기</button>
                <img :src="this.url"/>
            </div>
            <div v-if="check==1">
                처리하는중...
